...
function renderRepairList(data, isAdmin) {
  ...
  list.innerHTML = data.map(item => {
    ...
    return `
      <div class="repair-item" style="position:relative; padding-right:80px;">
        <h4 style="margin:0 0 8px 0; font-size:18px;">${item.Equipment} x ${item.Quantity}</h4>
        <p style="margin:0 0 5px 0; font-size:14px; color:#ddd;">描述：${item.Note||'無'}</p>
        <p style="margin:0 0 3px 0; font-size:13px; color:#aaa;">報修人：${item.Reporter||'匿名'}</p>
        <p style="margin:0; font-size:12px; color:#888;">時間：${timeStr}</p>
        ${isAdmin ? `<button class="btn-resolve" onclick="resolveRepair(${item.id})" style="position:absolute; right:15px; top:50%; transform:translateY(-50%); padding:8px 12px; font-size:12px; white-space:nowrap;">已修復</button>` : ''}
      </div>
    `;
  }).join('');
}
...
async function fetchRepairHistory() {
  ...
  if (cacheRepairHistory && (now - lastFetchRepairHistory < CACHE_TIMEOUT)) {
    ...
    renderRepairHistoryItems(cacheRepairHistory);
    return;
  }
...
function renderRepairHistoryItems(data) {
...
async function fetchBorrowHistory() {
  ...
  if (cacheHistory && (now - lastFetchHistory < CACHE_TIMEOUT)) {
    renderBorrowHistoryItems(cacheHistory);
    return;
  }
